...
- `coding-workflow`, `idea-shaping`, `session-handoff-load`, `session-handoff-save`, `team-memory`, and `writing-skills` are user-only skills: they require explicit user invocation and include `disable-model-invocation: true` so the model cannot invoke them on its own.
